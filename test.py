...
g_1_12 = torch.tensor(10.0, requires_grad=True)  # Changed initial value
...
G_b = torch.tensor([
    ...
    [-A * g_1_11 / A**2, -A * g_1_21 / A**2, (g_1_11 + g_1_21) / A**2, 0, 0],
    [-A * g_1_12 / A**2, -A * g_1_22 / A**2, 0, (g_1_12 + g_1_22) / A**2, 0],
...
i_eq = v_x * G_eq
x_free = compute_x(beta, G, i_nudge, i_eq)
...
beta_nudge = 10e-7
i_nudge = - A/B * beta_nudge * x_free[3]
i_equivalent = (v_x-G_inv[4,3]*i_nudge)/G_inv[4,4]

x_beta_new = compute_x(beta, G, i_nudge, i_equivalent)

#without the amplification
Vf = (x_free[3]/B - A * x_free[0]/B) ** 2
Vn = (x_beta_new[3]/B - A * x_beta_new[0]/B) ** 2
Pf = A * A * B * x_free[0]**2 - 2*A*B*x_free[0] * x_free[3] + x_free[3]**2
Pn = A * A * B * x_beta_new[0]**2 - 2*A*B*x_beta_new[0] * x_beta_new[3] + x_beta_new[3]**2
...
expression_rhs3 = 1 / (2*beta_nudge) * (Pn - Pf) 
...
for beta_nudge in beta_nudge_values:
    i_nudge = - beta_nudge * x_free[3]
    i_equivalent = (v_x-G_inv[4,3]*i_nudge)/G_inv[4,4]
    x_beta_new = compute_x(beta, G, i_nudge, i_equivalent)
    Vf2 = (x_free[3] * (B/A) - A * x_free[0] * (B/A)) ** 2
    Vn2 = (x_beta_new[3] * (B/A) - A * x_beta_new[0] * (B/A)) ** 2
    
    Vf = (A * v_x - A * x_free[0]) ** 2
    Vn = (A * v_x - A * x_beta_new[0]) ** 2
